...
{
    if (N == 0 || K == 0) {
        KLEIDIAI_DEBUG_LOG("MlasGemmPackB one of N or K is 0, falling back to MLAS.");
        return false;
    }
...
--*/
{
    ...
    if (Data[0].BIsPacked) {
        ...
        MlasTrySimpleParallel(ThreadPool, BatchSize, [&](ptrdiff_t batch_idx) {
            ...
            KLEIDIAI_KERNEL_LOG("kai_run_lhs_pack_f32p2vlx1_f32_sme" << " M=" << M << " K=" << K << " mr=" << mr << " kr=" << kr << " sr=" << sr);
