 ...
 public:
  ...
  Status PrePack(const Tensor& tensor, int input_idx, AllocatorPtr alloc,
    ...
    if (input_idx == GetBIdx()) {
      ...
      // Only handle 2D (or 1x2D) here
      if (!(b_shape_.NumDimensions() == 2 || (b_shape_.NumDimensions() == 3 && b_shape_[0] == 1))) {
        can_use_dynamic_quant_mlas_ = false;
      }

      // --- Decide whether to FORCE the decomposed path to route into KAI MlasGemmBatch ---
      // KAI integer->float kernel limitations: no per-column scale, no per-column zero-points.
      // We only check these once we know N (and after considering transposition).
      size_t K = static_cast<size_t>(b_shape_[0]);
      size_t N = static_cast<size_t>(b_shape_[1]);
      if (IsBTransposed()) std::swap(K, N);

      bool b_scale_is_per_column = false;
      if (b_scale_tensor) {
        const int64_t scale_elems = b_scale_tensor->Shape().Size();
        b_scale_is_per_column = (static_cast<size_t>(scale_elems) == N);
      }

      bool b_zp_is_per_column = false;
      if (b_zp_constant_tensor) {
        const int64_t zp_elems = b_zp_constant_tensor->Shape().Size();
        b_zp_is_per_column = (static_cast<size_t>(zp_elems) == N);
      }
      // If zp is dynamic input (not constant), it may be per-column → treat as per-column.
      const auto input_defs = Info().node().InputDefs();
      const bool b_zp_input_exists = input_defs.size() > IN_B_ZERO_POINT && input_defs[IN_B_ZERO_POINT]->Exists();
      if (!b_zp_constant_tensor && b_zp_input_exists) {
        b_zp_is_per_column = true;  // conservative
      }

      const bool prefer_kai_via_mlas_gemm =
          CPUIDInfo::GetCPUIDInfo().HasArm_SME()    // KAI available
          && b_scale_available                      // we still need scale
          && !b_scale_is_per_column                 // KAI: scalar scale only
          && !b_zp_is_per_column;                   // KAI: no per-column ZP

      if (prefer_kai_via_mlas_gemm) {
        // IMPORTANT:
        //  - Force decomposed path (so DynamicQuantizeMatMul will call MatMulIntegerToFloatBase::ComputeCommon)
        //  - Avoid packing entirely (we want BIsPacked == false for our KAI shim)
        can_use_dynamic_quant_mlas_ = false;
        is_packed = false;
        return Status::OK();
      }

      // If we cannot use the fused DynamicQGemm and we did NOT choose KAI, fall back to the base prepack
      // (this packs for MLAS MatMulInteger path, which KAI cannot consume).
      if (!can_use_dynamic_quant_mlas_) {
...
Status MatMulIntegerToFloat::PrePack(const Tensor& tensor, int input_idx, AllocatorPtr alloc,
  ...
  if (b_scale_available) {
    ...
    for (size_t i = 0; i < sz; ++i) { if (s[i] < 0.f) { scales_ok = false; break; } }
  }
